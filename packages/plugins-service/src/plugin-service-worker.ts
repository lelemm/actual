...
const fileList = new Map<string, Uint8Array<ArrayBuffer>>();
    ...
    channel.port1.onmessage = (messageEvent: MessageEvent<PluginFile[]>) => {
      ...
      if (responseData && Array.isArray(responseData)) {
        responseData.forEach(file => {
          fileList.set(`${slug}/${file.name}`, decodePluginFile(file));
        });
      }
...
function rewriteManifestPublicPath(
  content: Uint8Array<ArrayBuffer>,
  slug: string,
) {
  try {
...
function decodePluginFile({
  content,
  encoding,
}: PluginFile): Uint8Array<ArrayBuffer> {
  if (encoding === 'base64') {
