import type { SyncServerPluginSecrets } from '@actual-app/plugins-core-sync-server/plugin';
...
async function getCredentials(options?: SecretOptions): Promise<{
  ...
  const applicationId = await options.secrets.get('applicationId');
  const secretKey = await options.secrets.get('secretKey');
...
async function request<T>(
  ...
  const headers: Record<string, string> = {
    Authorization:
      authHeaderOverride ?? (await getAuthorizationHeader(secretOptions)),
...
export const enableBankingService = {
  async isConfigured(options?: SecretOptions): Promise<boolean> {
    if (!options) {
      return false;
    }
