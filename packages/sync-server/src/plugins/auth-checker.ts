...
import { getSession, getUserInfo, isAdmin } from '#account-db';

const TOKEN_EXPIRATION_NEVER = -1;
const MS_PER_SECOND = 1000;

type AuthLevel = 'anonymous' | 'authenticated' | 'admin';
type PluginUser = {
  id: string;
  role: 'admin' | 'user';
  username?: string;
 ...
 */
async function extractUserFromHeaders(
  headers: IncomingHttpHeaders,
): Promise<PluginUser | null> {
  // Get token from headers (case-insensitive)
  const token =
    getHeaderValue(headers, 'x-actual-token') ||
    getHeaderValue(headers, 'X-ACTUAL-TOKEN');
  ...
  try {
    ...
    const session = getSession(token) as SessionInfo | null;
    ...
    const user = getUserInfo(userId) as DbUser | null;
...
function checkAuth(
  ...
  console.warn(
    `Unknown auth level: ${String(authLevel)}, defaulting to 'authenticated'`,
  );
...
export type { AuthLevel, PluginUser, RouteConfig, RouteManifest };
