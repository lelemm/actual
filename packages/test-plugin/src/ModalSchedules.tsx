...
export function ModalSchedules({ context }: ModalSchedulesProps) {
    ...
    const fetchSchedules = async () => {
        ...
        try {
            ...
            console.log('Schedules fetched:', result.data);
            ...
            <View style={{ padding: 20, minWidth: 600 }}>
                <View>
                    <Text style={{ marginBottom: 15 }}>
                        This modal demonstrates AQL querying from the host app. Click the button below to fetch all schedules:
                    </Text>
                            ...
                            <View style={{
                                ...
                                <View style={{ gap: 10 }}>
                                    {schedules.map((schedule, index) => (
                                        <View key={schedule.id || index} style={{
                                            padding: 12,
                                            borderRadius: 4,
                    ...
                    {!loading && schedules.length === 0 && (
                        <View style={{
                            ...
                            <Text style={{ color: '#666' }}>
                                {schedules.length === 0 && !loading ? 'No schedules found or none fetched yet.' : ''}
                            </Text>
                        </View>
                    )}
