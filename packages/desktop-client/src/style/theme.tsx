...
import type { DarkTheme } from 'loot-core/types/prefs';
...
export function getThemeOptions(
  ...
  const pluginOptions = getPluginThemesFn
    ? getPluginThemesFn().map(
  ...
  const savedPluginOptions = savedPluginThemes
...
function ThemeStyleWithPlugins() {
  ...
  useEffect(() => {
    ...
    if (activeTheme === 'auto') {
      const darkTheme =
        builtInThemes[darkThemePreference as keyof typeof builtInThemes] ||
        builtInThemes.dark;
  ...
  useEffect(() => {
    ...
    if (isBuiltInTheme || hasLoadedPluginTheme) {
      const baseColors =
        builtInThemes[activeTheme as keyof typeof builtInThemes]?.colors || {};
      setThemeColors(getThemeColors(activeTheme, baseColors));
    }
    ...
    if (activeTheme === 'auto') {
      ...
      if (isBuiltInTheme) {
        ...
        if (savedTheme) {
          ...
          const savedBaseTheme = savedTheme.baseTheme || 'light';
          const baseThemeColors =
            builtInThemes[savedBaseTheme as keyof typeof builtInThemes]
