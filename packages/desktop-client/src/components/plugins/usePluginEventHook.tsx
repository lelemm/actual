...
type EventHookArgs<T extends PageKey, K extends EventHookKey<T>> =
...
export function usePluginEventHook<T extends PageKey>(defaultPage: T) {
  ...
  function pluginHook<K extends EventHookKey<T>>(eventName: K, eventArgs: EventHookArgs<T, K>): void;
  function pluginHook<K extends EventHookKey<T>>(eventName: K, eventArgs?: EventHookArgs<T, K>) {
    if (plugins.length === 0) return;

    plugins.forEach((plugin) => {
      const eventHooks = plugin.hooks?.[defaultPage]?.eventHooks as EventHooks<T> | undefined;

      if (eventHooks && typeof eventHooks[eventName] === 'function') {
        try {
          if (eventArgs !== undefined) {
            (eventHooks[eventName] as (args: EventHookArgs<T, K>) => any)(eventArgs);
          } else {
            (eventHooks[eventName] as () => any)();
          }
        } catch (error) {
          console.error(`Error in plugin: ${plugin.name}, event hook: ${String(eventName)}`, error);
        }
      }
    });
  }
