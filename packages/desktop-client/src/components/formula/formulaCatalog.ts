...
> {
  return {
    ...
    REGEXREPLACE: {
      ...
      modes: ['query', 'transaction'],
      description: t(
        'Replaces text matching a regular expression. Write the pattern as a regex literal /expression/flags (e.g. /[0-9]+/g). The replacement supports capture groups like $1.',
      ),
      parameters: [
        { name: 'text', description: t('Text to search') },
        {
          name: 'pattern',
          description: t('Regular expression literal, e.g. /[0-9]+/g'),
        },
        {
          name: 'replacement',
          description: t(
            'Replacement text, supports $1, $2 (optional, default "")',
          ),
        },
      ],
    },
