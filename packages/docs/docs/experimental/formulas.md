...
| `REGEXREPLACE`                  | Query, Rules | Replaces text matching a regular expression. Write the pattern as a regex literal `/expression/flags` (e.g. `/[0-9]+/g`). Replacement supports capture groups like `$1`. | `=REGEXREPLACE(imported_payee, / \d{4}$/, "")`                                                                                | `replacement` (default `""`)                                                                                    |
