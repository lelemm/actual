...
The repository includes a dummy bank sync plugin package for validating the plugin flow without external provider credentials:

- `packages/bank-sync-plugin-dummy`

It builds a zip with a manifest, sync-server routes, deterministic dummy accounts, deterministic transactions, and a small frontend account-linking screen.

The repository also includes a Pluggy.ai provider plugin package that shows a real provider implementation using the same MVP shape:

- `packages/bank-sync-plugin-pluggy.ai`

This minimal MVP does not include GoCardless, SimpleFIN, Enable Banking, or example-plugin provider implementations.

The provider package builds a zip containing `manifest.json`, frontend assets, and sync-server code.
